...
const Merge = () => {
  ...
  const mergePDFs = async () => {
    ...
    const pdfDocs = await Promise.all(files.map(file => {
  ...
  return (
    <div className={`flex h-[100vh] ml-10 md:ml-20 flex-col  bg-[#F5F7FA] items-center justify-between py-16  ${inter.className}`}>
   
      {!files ? (
        <div className="mx-auto rounded-lg text-center py-16  ">
          <h2   class="text-3xl md:text-[40px] lg:text-[60px]  font-bold mb-8 text-gray-800">Merge PDF Files</h2>
              ...
              {files?.length > 0 &&
                files?.map((file) => (
                  <div key={file.name} className="mx-2 overflow-hidden">
                   <embed className=" overflow-y-hidden" src={URL.createObjectURL(file)} type="application/pdf" width="100%" height='300px'>
                   </embed>
                    <p>{file.name}</p>
                  </div>
                ))}
